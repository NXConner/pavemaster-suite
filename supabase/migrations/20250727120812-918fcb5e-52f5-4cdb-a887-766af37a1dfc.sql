-- Database Migration: Create Core Business Tables
-- This creates the essential tables for the Pavement Performance Suite
...
CREATE POLICY "Authenticated users can view equipment" ON equipment_assets
    FOR SELECT USING (auth.role() = 'authenticated');
...
CREATE POLICY "Users can view their time tracking" ON time_tracking
    FOR SELECT USING (
        user_id = auth.uid() OR
        EXISTS (
            SELECT 1 FROM user_profiles 
...
CREATE POLICY "Users can record their time" ON time_tracking
    FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "All users can report safety incidents" ON safety_logs
    FOR INSERT WITH CHECK (reported_by = auth.uid());

CREATE POLICY "Users can view safety logs" ON safety_logs
    FOR SELECT USING (
