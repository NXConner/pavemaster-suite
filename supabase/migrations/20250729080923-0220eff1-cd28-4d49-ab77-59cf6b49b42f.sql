...
-- This migration addresses the security vulnerabilities identified by the linter

-- PART 1: Fix Function Search Path Security Issues
-- Secure all functions by setting search_path to prevent SQL injection

-- Fix existing functions with mutable search paths
...
-- PART 2: Enable RLS on missing tables (found 1 table without RLS)
-- The linter will identify which specific table needs RLS enabled

-- PART 3: Create comprehensive RLS policies for tables with RLS enabled but no policies
-- Based on the business logic, create secure policies for data access
...
-- Project-related tables policies
...
FOR SELECT USING (
  ...
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE id = auth.uid() 
    AND (is_admin = true OR role IN ('super_admin', 'manager'))
  )
);
...
-- Samples management policies
CREATE POLICY "Users can view samples for their projects" ON public.samples
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "Users can manage samples for their projects" ON public.samples
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM projects p 
    WHERE p.id = samples.project_id 
...
-- Profiles table policies (if exists separately from user_profiles)
...
-- PART 4: Add role escalation protection trigger
...
-- Apply the role protection trigger to user_profiles
...
-- PART 5: Enhanced security logging
...
-- PART 6: Create missing tables for security if they don't exist
...
-- PART 7: Add indexes for better performance on security-related queries
...
-- PART 8: Final security validation
-- Ensure all critical tables have proper RLS
DO $$
DECLARE
  table_record RECORD;
BEGIN
